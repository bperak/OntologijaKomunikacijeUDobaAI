...
**Praktikum.** Postupak iz 10.1 pokreće se u cijelosti, i svaka se njegova odluka prijavljuje uz rezultat. (1) Fiksiraj popis jedinica u jednoj datoteci svoga rada — u repozitoriju knjige to je `data/leksemi.txt`, koji sastavljaš po vlastitome kriteriju (popis od 125 emocionalnih jedinica iz ovoga poglavlja dolazi iz navedenih izvora i u repozitorij se ne prenosi) — i zapiši kontrolni zbroj popisa; popis se između dvaju mjerenja ne mijenja. (2) Provedi ugrađivanje modelom **Qwen3-Embedding** (**4.096 dimenzija**) i spremi vektore zajedno s imenom i verzijom modela: dimenzija je mjerena činjenica o modelu, ne o jeziku. (3) Izračunaj kosinusnu srodnost uz L2-normalizaciju i **ispiši raspon srodnosti**; to je dijagnostika sabijanja udaljenosti u 4.096 dimenzija, a ne ukras. (4) Odaberi broj skupina *k*, prag i sjeme i **zapiši ih prije nego što pogledaš rezultat** — broj skupina je ulazna odluka analitičara, a ne izlaz postupka. (5) Ponovi isti postupak s drugim sjemenom, s drugim poduzorkom jedinica i s drugom verzijom modela ugrađivanja; zapiši koliko se pripadnosti promijenilo. (6) Nacrtaj prikaz, ali ga čitaj kao kazalo: svaku tvrdnju pročitanu sa slike provjeri u punome prostoru — na matrici srodnosti i na popisu najbližih susjeda. (7) Za svaku brojku u izvješću navedi **vrstu dokaza i datum** po obrascu evidencijske tablice iz 10.6.1 — mjereno, procjena ili izvedeno. (8) Napiši i **negativan nalaz**: ako se skupine preslože pri promjeni verzije modela, nalaz je o jednome modelu, a ne o jeziku. Izlaz praktikuma nije „značenje jedinica", nego zapis postavka: matrica srodnosti, popis najbližih susjeda, pripadnost skupinama i mjera stabilnosti — uz popis onoga što je u postupku odlučeno. Objava odluka dio je rezultata, a ne popratna bilješka: tko ne zapiše mjeru srodnosti, način normalizacije, broj skupina i verziju modela, ne može ponoviti nalaz — a ni provjeriti je li on o jeziku ili o jednome modelu.

**Ako ne radi — tri najčešće greške.** *Prva:* popis jedinica promijenjen je između dvaju mjerenja — dodana ili izbačena jedinica, druga lematizacija, drugi filter — pa usporedba mjeri popis, a ne model; kontrolni zbroj popisa je zato prvi redak zapisa, a ne formalnost. *Druga:* pomiješane su dvije linije ugrađivanja (gola lema i prosjek njezinih pojavnica), pa se razlika među rezultatima pripisuje modelu, a nastaje iz odluke o ulazu — jedna se linija odabire i prijavljuje. Ista opreza vrijedi i za mjeru udaljenosti: euklidska udaljenost, skalarni produkt i kosinus daju različite popise najbližih susjeda, pa se mjera piše u zapis, a ne prešućuje. *Treća:* ako su sve vrijednosti srodnosti blizu jedinice ili izvan raspona, matrica nije izračunata kao umnožak normaliziranih vektora (`S = jedinicni(U) @ jedinicni(U).T`); normalizacija bez umnoška daje jediničnu normu, a ne kosinus, pa mjera ne mjeri srodnost. Isto vrijedi za čitanje skupina iz dvodimenzionalnoga prikaza bez provjere u punome prostoru: nalaz koji postoji samo na jednoj slici nije nalaz. Za svaku tvrdnju pročitanu sa slike navedi mjesto u matrici na kojemu je provjerena — bez toga je prikaz kazalo koje nitko nije slijedio.
