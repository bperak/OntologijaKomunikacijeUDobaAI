...
❓ **Otvoreno za provjeru u ovoj datoteci:** (1) pune bibliografske jedinice za Anthropic 2024 (MCP), Google 2025 (A2A) i Google 2025 (AP2) / Coinbase 2025 (x402) — navesti izdanje i datum objave prema autorovoj bibliografiji; (2) Knight First Amendment Institute (2025) — potvrditi naslov i broj razina autonomije prije citiranja pojedine razine; (3) brojka ~700 agenata u koordiniranom napadu ostaje **procjena prema izvještajima** (Fortune, CNN, Taipei Times; `data/fakti.csv`: `openai_agents_swarm`) i ne smije se navesti kao mjerenje; (4) nalaz GreyNoisea (2026, 9. rujna) — 395 organizacija i 11 ciljeva u 26 sekundi — ostaje **mjereno** (`data/fakti.csv`: `papercut_orgs`, `papercut_targets_26s`).
